...
### ۲. شناسایی خودکار در شبکه (UDP Wi-Fi Beacon & Radar)
- بدون نیاز به وارد کردن تنظیمات پیچیده، گوشی‌های حاضر در یک شبکه وای‌فای میزبان را از طریق برادکست‌های دوره‌ای شناسایی کرده و با یک ضربه متصل می‌شوند.
- قابلیت **اتصال مستقیم با آی‌پی (Direct IP)** برای شبکه‌های خاص سازمانی یا موارد عدم پشتیبانی از برادکست.

### ۳. صف پخش و پلی‌لیست مشترک (Shared Queue & Voting)
...
### ۴. کنسول کنترل از راه دور (Multi-Room Remote Control)
...
### ۵. رابط کاربری جذاب، راست‌چین و مدرن (Persian RTL Design)
...
- `ACCESS_WIFI_STATE` و `CHANGE_WIFI_MULTICAST_STATE`: جهت برادکست و دریافت امواج بیکن میزبان روی وای‌فای محلی.
